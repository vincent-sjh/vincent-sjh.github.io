...
在五道口技术学院某抽象院系的某水课的一次作业中，出现了有趣的情况。

作业要用某工具监测某可执行文件运行过程中的状态和数据，最后通过数据推测得到助教提前设置好的两个信息。

当然，看到这里你肯定会想到，提供给同学们下载的是编译好的可执行文件，而不是源代码，不然可以直接阅读获得答案。

同时，这个可执行文件运行需要一个参数，助教规定每个同学将参数设置为自己的学号。

这显然是为了保证同学们的答案不全相同来防止抄袭，但我们也可以推测出正确答案一定是输入参数的唯一函数。

## 使用objdump进行反汇编

```shell
//-d 选项指示 objdump 对目标文件中包含可执行代码的节（section）进行反汇编，将机器码转换为对应架构的汇编指令。
//-C 选项用于解析编译器对符号名进行的名称修饰（name mangling），将修饰后的符号名转换为原始可读的函数名（如 foo(int, int)）
objdump -d -C ./main
```
...
得到`lipsum()`函数和`main`函数的等效源代码，现在的反编译水平得出的源码可读性还比较差，我们硬看还看不出门道。
...
一次有趣的逆向工程之旅就这样结束了，可以给逆向工程（`Reverse Engineering`）下一个定义：分析一个已有系统、程序、设备或对象的结构、功能和行为，推导出其设计原理、实现细节或源代码的过程。通常，逆向工程的目标是理解系统的内部工作机制，以便复制、改进、修复、兼容或分析其功能，而不依赖原始设计文档或源代码。在计算机领域，逆向工程常用于软件分析、硬件分析、安全研究和兼容性开发。
...
3. **Ken Thompson（UNIX 创始人之一）**
   ...
   “要真正理解一个程序，你必须能够追溯它的每一行代码。逆向工程让我们看到隐藏在二进制背后的真相。”
